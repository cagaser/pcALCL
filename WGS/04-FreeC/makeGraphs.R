...
for (i in c(1:22,'X','Y')) {
	png(filename = paste("chr", i,".png",sep = ""), width = 1100, height = 1100,
    	units = "px", pointsize = 10, bg = "white", res = NA)
	par(mar=c(3,3,1,1))
	maxLevelToPlot <- 3
	for (x in c(1:length(ratio$Ratio))) {
		if (ratio$Ratio[x]>maxLevelToPlot) {
			ratio$Ratio[x]=maxLevelToPlot;
		}
	}
	tt <- which(ratio$Chromosome==i)
	if (length(tt)>0) {
	 plot(ratio$Start[tt],ratio$Ratio[tt]*ploidy,ylim = c(0,maxLevelToPlot*ploidy),xlab = paste ("position, chr",i),ylab = "normalized copy number profile",pch = 19,col = colors()[88])
	 tt <- which(ratio$Chromosome==i  & ratio$CopyNumber>ploidy )
	 points(ratio$Start[tt],ratio$Ratio[tt]*ploidy,pch = 19,col = colors()[136])
	
	tt <- which(ratio$Chromosome==i  & ratio$Ratio==maxLevelToPlot & ratio$CopyNumber>ploidy)	
	points(ratio$Start[tt],ratio$Ratio[tt]*ploidy,pch = 19,col = colors()[136],cex=4)
	 
	tt <- which(ratio$Chromosome==i  & ratio$CopyNumber<ploidy & ratio$CopyNumber!= -1)
	 points(ratio$Start[tt],ratio$Ratio[tt]*ploidy,pch = 19,col = colors()[461])
	 tt <- which(ratio$Chromosome==i)
...
if (length(args)>5) {
	...
	png(filename = paste(args[6],".png",sep = ""), width = 1180, height = 1180,
	    units = "px", pointsize = 5, bg = "white", res = NA)
	plot(1:10)
	op <- par(mfrow = c(5,5))
	par(mar=c(1,1,1,1))
	for (i in c(1:22,'X','Y')) {
	    ...
	    if (length(tt)>0){
		lBAF <-BAF[tt,]
		plot(lBAF$Position,lBAF$BAF,ylim = c(-0.1,1.1),xlab = paste ("position, chr",i),ylab = "BAF",pch = 19,col = colors()[1])

		tt <- which(lBAF$A==0.5)		
		points(lBAF$Position[tt],lBAF$BAF[tt],pch = 19,col = colors()[92])
		tt <- which(lBAF$A!=0.5 & lBAF$A>=0)
		points(lBAF$Position[tt],lBAF$BAF[tt],pch = 19,col = colors()[62])
		tt <- 1
		pres <- 1
